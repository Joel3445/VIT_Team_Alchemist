...
func load_experiment(dsl: Dictionary, _register_mgr: Node) -> void:
	...
	if step_map.has(start_id):
		...
		print("GLAM started at: ", start_id)
...
func try_advance(action_id: String, register_mgr: Node) -> void:
	...
	var wire_on = wire.get("on", "")
	if wire_on == null: wire_on = ""
	if wire_on != action_id:
		print("GLAM: action '", action_id, "' doesn't match wire '", wire_on, "'")
		return
	var cond_id = wire.get("condition", "")
	if cond_id == null: cond_id = ""
	if cond_id != "" and not register_mgr.check_condition(cond_id, world_state):
		print("GLAM: condition '", cond_id, "' not met")
		return
	var next_id = wire.get("next", "END")
	if next_id == null: next_id = "END"
	if next_id == "END":
		current_step = {}
